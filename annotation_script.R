...
save(opts$ann, file = 'opts_da_used_for_analysis')
message('... saved.')
...
data_annotation$probes_direct$annotations <- master_annotator(
  ...
  des_exp_id_col = opts$ann$des_exp_id_col,
...
data_annotation$gemma$candidate_publ_for_gemma <- 
  unique(data_annotation$probes_direct$leftover[[opts$ann$input_exp_id_col]])[unique(data_annotation$probes_direct$leftover[[opts$ann$input_exp_id_col]]) %nin% unique(data_annotation$probes_direct$finalized[[opts$ann$input_exp_id_col]])]

data_annotation$gemma$candidate_publ_for_gemma <- 
  unique(subset(
    x = data_annotation$descriptions, 
    subset = data_annotation$descriptions[[opts$ann$des_exp_id_col]] %in% data_annotation$gemma$candidate_publ_for_gemma, 
    select = c(opts$ann$des_exp_id_col, opts$ann$des_platform_col))) 
...
data_annotation$gemma$gemma_platforms <- download_platforms_from_gemma(unique(data_annotation$gemma$candidate_publ_for_gemma[[opts$ann$des_platform_col]])) ### !!! changed from data_annotation$gemma$candidate_publ_for_gemma$Assay
message('... downloaded.')
...
data_annotation$gemma$pubs_found_in_gemma <- subset(
  ...
  subset = toupper(data_annotation$gemma$candidate_publ_for_gemma[[opts$ann$des_platform_col]]) %in% names(data_annotation$gemma$gemma_platforms[!is.na(data_annotation$gemma$gemma_platforms)])) ### !!! changed from data_annotation$gemma$candidate_publ_for_gemma$Assay
...
data_annotation$gemma$annotation_raw <- get_gemma_annotations_for_data(
  ...
  input_paper_id_col = opts$ann$input_exp_id_col, 
...
save(probes_direct, file = paste0(opts$ann$folder, '/identifier_centered_analysis'))
message('... saved. Please remember to check quality of data - use "qa" element of saved list.')
...
save(full_finalized, file = paste0(opts$ann$folder, '/full_finalized_data'))
message('... saved. Please remember to check quality of data - use "qa" element of saved list.')

message('Saving final leftover data as true_leftovers_data...')
true_leftovers <- data_annotation$true_leftovers
save(true_leftovers, file = paste0(opts$ann$folder, '/true_leftovers_data'))
message('... saved. Please remember to check quality of data - use "qa" element of saved list.')
